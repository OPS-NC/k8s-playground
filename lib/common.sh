...
# kube-proxy is optional on BOTH (Cilium replaces it in eBPF, the default of both labs); only
# the way the bootstrap drops it differs.
#
...
# There is no equivalent on Talos: `CNI` and `KUBE_PROXY_REPLACEMENT` are read from `lab.env`
# there, so they stay INTENTS — they must not be edited after the bootstrap.
read_cluster_env() {
  sed -n "s/^[[:space:]]*$1=//p" \
