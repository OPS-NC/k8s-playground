...
"$MC" mb --ignore-existing "${MC_ALIAS}/cnpg-backups" >/dev/null
POLICY="$(mktemp)"; cat > "$POLICY" <<'JSON'
{ "Version":"2012-10-17","Statement":[
  {"Effect":"Allow","Action":["s3:*"],"Resource":["arn:aws:s3:::cnpg-backups","arn:aws:s3:::cnpg-backups/*"]} ]}
JSON
"$MC" admin policy create "$MC_ALIAS" cnpg-backups-rw "$POLICY" >/dev/null 2>&1 || true
...
"$MC" admin user add "$MC_ALIAS" cnpg-backup "$SK" >/dev/null 2>&1 || true
"$MC" admin policy attach "$MC_ALIAS" cnpg-backups-rw --user cnpg-backup >/dev/null 2>&1 || true
