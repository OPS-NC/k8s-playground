...
| `calico` · `flannel` · `none` | MetalLB | ✅ (for `none`, once your own CNI has made the nodes `Ready`) |
...
| A **working CNI**, nodes `Ready` | MetalLB is an ordinary workload: with no pod network the controller never gets an IP | `kubectl get nodes` |
...
Chart `metallb/metallb` **`0.16.1`**, overridable per run
...
The announcement itself is identical: same chart, same pool, same interface, same node selection.
Two things differ, and only the first one matters.
...
# 4. Controller AND speakers — the controller allocates, the speakers announce
...
kubectl -n envoy-gateway-system get svc                 # the Gateway took the FIRST IP of the pool
kubectl get servicel2status -A -o wide                  # who announces what
```
...
|---|---|---|
| The script fails on "Cilium already announces" | you are on `CNI=cilium` — the lab default | that is the guard rail: Cilium already does this |
...
| ARP does not resolve at all | announcement on the NAT card, or `interfaces` written as a regex | it must read `enp0s8`, **not** `^enp0s8$` |
